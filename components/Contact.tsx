        ...
        <div className="lg:col-span-5">
          ...
          <Reveal delay={0.2} className="mt-14 grid gap-10 text-bone/70 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="label mt-8 mb-3">Phone</p>
              <a href={`tel:${site.phone.replace(/\s/g, "")}`} className="link">{site.phone}</a>
            </div>
          </Reveal>
        </div>
          ...
          <form onSubmit={onSubmit} noValidate className="grid gap-10">
            ...
            <div className="flex flex-wrap items-center justify-between gap-6">
              <button
                ...
                className="label border border-bone/30 px-8 py-4 text-bone transition-[border-color,background-color,color] duration-700 ease-out-slow hover:border-bronze hover:bg-bronze hover:text-ink disabled:opacity-50"
              >
                {status === "sending" ? "Sending…" : "Send message"}
              </button>
              <p className="label text-bronze" role="status" aria-live="polite">
